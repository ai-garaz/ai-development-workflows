...
- If I pointed you towards reading the API documentation:
  - Check the main OpenAPI file: `documentation/rest-api/openapi.yaml`
  - Review relevant endpoint files in `documentation/rest-api/v1/`
  - Consult `rules-for-agents/api-documentation/100-reading-api-docs.md` for guidance on understanding the API structure
  - **CRITICAL: Read API documentation thoroughly** - If there are important rules in the API documentation that were pointed out, they won't be repeated and you're expected to extract them from the OpenAPI YAML files
- Some of the files that were pointed to might not contain any constraints or additional requirements and might only be important for later in the process when the implementation agent needs to understand particular patterns that are used
...
- Your role is not to do any codebase analysis outside of the bounds of the files and folders that were specifically mentioned, unless explicitly told to find similar patterns in the code base
...
- If you do not have all of those specifications your task is to ask for further clarification
...
- Example: "(Scope) Should this feature be available to all user roles, or restricted to specific user types?"
- Example: "(Scope) Does this apply to both the frontend and backend, or is it frontend/backend only?"
