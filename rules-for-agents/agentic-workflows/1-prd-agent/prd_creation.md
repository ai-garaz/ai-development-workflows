...
- Output Format: A structured PRD document following the template below, always in English language
...
Either the task code has been provided (e.g., `RP-001`), or it should have been determined in the previous phase.

The feature name should have been determined during the context gathering phase.

Example: `RP-001-user-authentication-flow`
...
- **Reference specific coding rules by name** (e.g., `rules-for-agents/coding/001-general-coding-rules.md`) instead of vague descriptions if you were pointed towards rules during context gathering phase
...
- Reference the relevant OpenAPI documentation in `documentation/rest-api/`
- Document which specific endpoints will be used or created
- Specify if new endpoints need to be designed (following `rules-for-agents/api-documentation/200-writing-api-docs.md`)

**Note**: Rather than duplicating API documentation, reference the existing OpenAPI YAML files, providing additional context that was gathered and is not contained in the documentation.
...
- [API Endpoint 1] - Location: documentation/rest-api/v1/[path]/[METHOD].yaml - Relevance to feature
- ...
